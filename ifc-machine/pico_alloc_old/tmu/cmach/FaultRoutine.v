...
Lemma init_init: forall (m: Mem.t (@Atom Z))  op ts tpc, cache_hit_mem m op ts tpc ->
let '(l1,l2,l3) := ts in
handler_initial_mem_matches op l1 l2 l3 tpc m.
Proof.
  unfold cache_hit_mem.
  intros.
  destruct (Mem.get_frame m Mem.cblock) eqn:E; try (elim H; fail).  
...
Proof.
  ...
  destruct (Mem.get_frame m Mem.cblock) eqn:E1.
...
Proof.
  ...
  simpl in *; unfold Mem.load;
  unfold Mem.cget, Atom in *; rewrite H.
      ...
      HT   (genApplyRule am)
           ...
           (fun m s => m = m0 /\
                       s = CData (Vint        1, handlerTag) :: (* [Some (...)] *)
                           CData (Vint        1, handlerTag) :: (* [Some l1] *)
                           CData (Vint (labToZ l1), handlerTag) ::
                           CData (Vint (labToZ l2), handlerTag) :: s0).
Proof.
...
:=
  ...
  | Some (None,    lpc) => CData (Vint 1, handlerTag) ::
                           CData (Vint 0, handlerTag) ::
                           CData (Vint (labToZ lpc), handlerTag) :: s0
  | Some (Some lr, lpc) => CData (Vint 1, handlerTag) ::
                           CData (Vint 1, handlerTag) ::
...
Proof.
  ...
  unfold Mem.load, Atom, Mem.cget in *; rewrite H; eauto.
Qed.
...
Lemma genFaultHandlerMem_spec_Some_None: forall lpc,
  valid_address Mem.cblock addrTagResPC m0 ->
  ar = Some (None, lpc) ->
  forall s0,
    HT genFaultHandlerMem
       (fun m s => m = m0 /\
                   s = listify_apply_rule ar s0)
       (fun m s => Mem.store Mem.cblock addrTagResPC (Vint (labToZ lpc),handlerTag) m0 =
                     Some m /\
...
Lemma genFaultHandlerMem_spec_Some_Some: forall lr lpc,
  valid_address Mem.cblock addrTagRes m0 ->
  valid_address Mem.cblock addrTagResPC m0 ->
  ar = Some (Some lr, lpc) ->
  forall s0,
    HT genFaultHandlerMem
       (fun m s => m = m0 /\
                   s = listify_apply_rule ar s0)
       (fun m s =>
        (exists m', Mem.store Mem.cblock addrTagRes (Vint (labToZ lr),handlerTag) m0
                    = Some m'
                 /\ Mem.store Mem.cblock addrTagResPC (Vint (labToZ lpc),handlerTag) m'
                    = Some m)
        /\ s = (Vint 1,handlerTag) ::: s0).
Proof.
...
Lemma load_store_old : forall {A} (m m':Mem.t A) b0 ofs0 v b ofs,
  Mem.store b0 ofs0 v m = Some m' ->
  (b, ofs) <> (b0, ofs0) ->
  Mem.load b ofs m = Mem.load b ofs m'.
Proof.
  unfold Mem.store, Mem.load; intros.
  destruct (Mem.get_frame m b0) eqn:E0; try congruence.
  destruct (Mem.block_eq b b0).
  - subst.
    destruct (upd_m ofs0 v l) eqn:Eu; inv H.
    rewrite E0.
    rewrite Mem.get_upd_frame_new.
    eapply update_list_Z_spec2; eauto.
    congruence.
  - destruct (upd_m ofs0 v l) eqn:Eu; inv H.
    rewrite Mem.get_upd_frame_old; auto.
Qed.
...
Definition handler_final_mem_matches' (olr: option T) (lpc: T) (m: memory) (m': memory) :Prop :=
  (match olr with
     | Some lr => cache_hit_read_mem m' (labToZ lr) (labToZ lpc)
     | None => exists t, cache_hit_read_mem m' t (labToZ lpc)
   end)
  (* Nothing else changed *)
  /\ update_cache_spec_rvec m m'.

Definition valid_address_vint b a (m: memory) :=
  exists i lbl, Mem.load b a m = Some (Vint i,lbl).

Lemma valid_address_vint_valid_address: forall b a m,
  valid_address_vint b a m -> valid_address b a m.
Proof.
  intros.
  destruct H as (i & lbl & H).
  eexists; eauto.
Qed.
Hint Resolve valid_address_vint_valid_address.

Lemma genFaultHandlerMem_spec_Some: forall olr lpc,
  valid_address_vint Mem.cblock addrTagRes m0 ->
  valid_address Mem.cblock addrTagResPC m0 ->
  ar = Some (olr, lpc) ->
  forall s0,
    HT genFaultHandlerMem
...
Proof.
  ...
  { unfold handler_final_mem_matches'.
    ...
    { cases olr; subst olr.
        ...
        + unfold handler_final_mem_matches'. intuition.
          - (* rewrite (ZToLab_labToZ_id t). *)
            ...
            repeat match goal with
              id : context[match ?x with |_ => _ end] |- _ => 
                    let E := fresh "Eq" in
                    destruct x eqn:E; try congruence
                   end.
            inv H0; inv H.
            rewrite Mem.get_upd_frame_new in *.
            inv Eq.
            clean_up_initial_mem.
            unfold Mem.cget in *.
            assert (x = l1) by (unfold Atom in *; congruence); subst.
            eapply chr_uppriv.
            (* Ugh ... *)
            * eapply tim_intro.
              jauto_set_hyps; intros.              
      ...
      { eapply HT_weaken_conclusion.
        + eapply genFaultHandlerMem_spec_Some_None;  eauto.
        + destruct HvalidRes as (i & lbl & L).
          unfold handler_final_mem_matches'. intuition.
          - clean_up_initial_mem.
            unfold Mem.cget, Mem.store, Mem.load in *.
            repeat match goal with
              id : context[match ?x with |_ => _ end] |- _ => 
                    let E := fresh "Eq" in
                    destruct x eqn:E; try congruence
            ...
            inv H0; inv H.
            assert (x = l) by (unfold Atom in *; congruence); subst.
            unfold cache_hit_read_mem.
            rewrite Mem.get_upd_frame_new.
            eexists.
            eapply chr_uppriv.
...
Lemma genFaultHandlerReturn_specEscape_Some: forall a l olr lpc,
  forall s0,
  HTEscape (a,l) genFaultHandlerReturn
       (fun (m : memory) (s : stack) =>
        handler_final_mem_matches' olr lpc m0 m /\
        s = (Vint 1, handlerTag) ::: CRet (Vint a, l) false false :: s0)
       (fun (m : memory) (s : stack) =>
...
Lemma faultHandler_specEscape_Some: forall a l olr lpc,
  valid_address_vint Mem.cblock addrTagRes m0 ->
  valid_address Mem.cblock addrTagResPC m0 ->
  ar = Some (olr, lpc) ->
  forall s0,
    HTEscape (a,l) (faultHandler fetch_rule_impl)
             (fun m s => m = m0 /\
                         s = (CRet (Vint a, l) false false::s0))
             (fun m s => ( s = s0 /\
                           handler_final_mem_matches' olr lpc m0 m
                         , Success )).
Proof.
...
Conjecture handler_correct : 
  ...
  forall  opcode vls pcl m reta retl c imem fhdl s,
    ...
    exists m' st pc priv, 
      runsToEscape (CState m fhdl imem (CRet (Vint reta, retl) false false::s) (0,handlerTag) true)
                   nil (CState m' fhdl imem st pc priv) /\ 
      match apply_rule (projT2 am) vls pcl with
        | Some (olr,lpc) => handler_final_mem_matches' olr lpc m m' 
                     /\ pc = (reta,retl)
                     /\ st = s
                     /\ priv = false
        | None => m' = m /\ pc = (-1,handlerTag) /\ priv = true
    end.
...
Theorem handler_correct_succeed :
  forall opcode vls pcl m a l s i olr lpc,
  forall (INPUT: cache_hit_mem m (opCodeToZ opcode) (labsToZs vls) (labToZ pcl))
    ...
    exists m',
    runsToEscape (CState m handler i (CRet (Vint a,l) false false::s) (0,handlerTag) true)
                 nil (CState m' handler i s (a,l) false) /\
    handler_final_mem_matches' olr lpc m m'.
Proof.
  intros.
  unfold cache_hit_mem in *.
  destruct (Mem.get_frame m Mem.cblock) eqn:E; try (intuition; fail).

  assert (valid_address_vint Mem.cblock addrTagRes m).
    inv INPUT. inv TAGR. 
    econstructor.
    unfold Mem.load; rewrite E.
    edestruct index_list_Z_valid; eauto. 
  assert (valid_address Mem.cblock addrTagResPC m).
  ...
  unfold cache_hit_mem; rewrite E; auto.
  apply code_at_id. 
  exists cache1. 
  inversion P3.  subst. 
  intuition.
...
Theorem handler_correct_fail : 
  ...
  forall (INPUT: cache_hit_mem m (opCodeToZ opcode) (labsToZs vls) (labToZ pcl))
    ...
    exists st,
    runsToEscape (CState m handler i (CRet raddr false false::s) (0,handlerTag) true)
                 nil (CState m handler i st (-1,handlerTag) true).
Proof.
  intros.   
