...
Definition memory : Type := Mem.t (@Atom Z).
...
Lemma HT_star: forall c P Q,
  ...
  exists mem1 stk1,
  Q mem1 stk1 /\
  star cstep (CState mem0 fh imem stk0 (n, handlerTag) true)
             nil
             (CState mem1 fh imem stk1 (n', handlerTag) true).
Proof.
  unfold HT.
  introv HTcPQ; intros.
  edestruct HTcPQ as [stk2 [mem2 [HQ RTE2]]]; eauto; clear HTcPQ.
...
Proof.
  unfold HTEscape.
  introv HTcPQ; intros.
  edestruct HTcPQ as [stk1 [mem1 [pc1 [priv1 RTE2]]]]; eauto; clear HTcPQ.
  exists stk1 mem1 pc1 priv1.
  destruct (Q mem1 stk1); intuition.
...
Proof.
  ...
  edestruct (HT_star _ _ _ H_HT) as [mem1 [stk1 [HQ Hstar1]]]; eauto.
  eapply code_at_compose_1; eauto.

  edestruct (HTEscape_star _ _ _ _ H_HTE) as [stk2 [mem2 [pc2 [priv2 Hlet]]]]; eauto.
  eapply code_at_compose_2; eauto.

  exists stk2 mem2 pc2 priv2.
  destruct (R mem2 stk2).
...
Proof.
  ...
  unfold HT. intros imem stk0 c0 fh0 n n' H_code_at HP neq.
...
Proof.
  intros.
  unfold HT in *. intros.
  edestruct (H imem) as [sk [cache [HQ R]]]. eapply H1.  eapply H2. eapply H3. 
  edestruct (H0 imem) as [sk' [cache' [HQ' R']]]; eauto.
...
Lemma PushCachePtr_spec: forall m0 s0,
  HT [PushCachePtr]
     (fun m s => m = m0 /\ s = s0)
     (fun m s => m = m0 /\ s = CData (Vptr Mem.cblock 0,handlerTag) :: s0).
Proof.
  repeat intro.
  do 2 eexists.
  intuition; subst. 
...
Lemma PushCachePtr_spec': forall P,
  HT [PushCachePtr]
     P
     (fun m s => head s = Some (CData (Vptr Mem.cblock 0,handlerTag)) /\
                            P m (tail s)).
Proof.
  repeat intro.
  exists ((CData (Vptr Mem.cblock 0, handlerTag))::stk0).
  exists mem0.
  simpl in H1.
  intuition; subst. 
...
Lemma load_spec: forall b ofs al v vl, forall m0 s0,
  Mem.load b ofs m0 = Some (v,vl) ->
  Mem.privilege_bit b = Kernel ->
  HT [Load]
...
Lemma push_cptr_spec: forall ofs, forall m0 s0,
  HT (push_cptr ofs)
     (fun m s => m = m0 /\ s = s0)
     (fun m s => m = m0 /\ s = CData (Vptr Mem.cblock ofs,handlerTag) :: s0).
Proof.
  intros.
...
Lemma push_cptr_spec': forall ofs P,
  HT (push_cptr ofs)
     P
     (fun m s => head s = Some (CData (Vptr Mem.cblock ofs,handlerTag)) /\
                            P m (tail s)).
Proof.
  intros.
  replace (push_cptr ofs) with ([PushCachePtr]++[Push ofs]++[Add]) by auto.
  ...
  simpl in *.
  exists ((CData (Vptr Mem.cblock ofs, handlerTag))::stk0).
  eexists.
  eexists.
  intuition. 
  simpl; eauto.
...
Lemma loadFrom_spec: forall ofs v vl, forall m0 s0,
  Mem.load Mem.cblock ofs m0 = Some (v,vl) ->
  HT (loadFrom ofs)
     (fun m s => m = m0 /\ s = s0)
     (fun m s => m = m0 /\ s = CData (v,handlerTag) :: s0).
Proof.
  ...
  eapply load_spec.
  eauto.
  apply Mem.privilege_bit_cblock.
Qed.

Lemma pop_spec: forall v vl, forall m0 s0,
  HT pop
     (fun m s => m = m0 /\ s = CData (Vint v,vl) :: s0)
     (fun m s => m = m0 /\ s = s0).
Proof.
  unfold HT.
  intros.
...
Definition valid_address b a (m: memory) :=
  exists v, Mem.load b a m = Some v.
...
Lemma valid_store: forall b a v m,
  valid_address b a m ->
  exists m', Mem.store b a v m = Some m'.
Proof.
  intros.
  destruct H as [v' H].
  unfold Mem.load, Mem.store in *.
  destruct (Mem.get_frame m b) eqn:E.
  unfold upd_m, read_m in *.
  destruct (a <? 0) eqn:Ea; try congruence.
  edestruct (@update_list_Z_Some _ v l a); iauto.
  apply Z.ltb_ge; auto.
  eapply index_list_valid; eauto.
  unfold upd_m in *; rewrite Ea in *.
  rewrite H0; eauto.
  congruence.
Qed. 
...
Lemma storeAt_spec: forall a v vl m s,
  HT (storeAt a)
     (fun m0 s0 => m0 = m /\
                   s0 = (v,vl) ::: s /\
                   valid_address Mem.cblock a m)
     (fun m1 s1 => s1 = s /\
                   Mem.store Mem.cblock a (v,vl) m = Some m1).
Proof.
  intros.
  eapply HT_compose.
...
Lemma storeAt_spec_wp: forall a vl Q,
  ...
  HT (storeAt a)
     (fun m0 s0 => s0 = vl ::: s /\
                   Mem.store Mem.cblock a vl m0 = Some m /\
                   Q m s)
     Q.
Proof.
  intros.
...
Lemma ite_spec: forall c t f P Pt Pf Qc Q,
  ...
  HT   t Pt Q ->
  HT   f Pf Q ->
  (forall m s, Qc m s -> (exists v l s', s = CData (Vint v,l) :: s' /\
                                         (v <> 0 -> Pt m s') /\
                                         (v =  0 -> Pf m s')))
  ->
  HT   (ite c t f) P Q.
...
Lemma ite_spec_specialized: forall v c t f Q, forall m0 s0,
  ...
  HT c (fun m s => m = m0 /\ s = s0)
       (fun m s => m = m0 /\ s = (Vint v,handlerTag) ::: s0) ->
  (v <> 0 -> HT t P Q) ->
  (v =  0 -> HT f P Q) ->
  HT (ite c t f) P Q.
Proof.
...
Lemma valid_address_upd: forall b a b' a' vl m m',
  valid_address b a m ->
  Mem.store b' a' vl m = Some m' ->
  valid_address b a m'.
Proof.
  unfold valid_address; intuition.
  destruct H.
  unfold Mem.load, Mem.store in *.
  destruct (Mem.block_eq b b').
  - subst.
    ...
    + rewrite e in *; erewrite update_list_spec; eauto.
    + erewrite <- update_list_spec2; eauto.
  - destruct (Mem.get_frame m b) eqn:E; try congruence.
    destruct (Mem.get_frame m b') eqn:E'; try congruence.
    destruct (upd_m a' vl l0) eqn:E0; try congruence.
    inv H0.
    erewrite Mem.get_upd_frame_old; eauto.
    rewrite E.
    eauto.
Qed.
...
Lemma ret_specEscape: forall pc pcl (P: memory -> stack -> Prop),
  HTEscape (pc,pcl) [Ret]
    (fun m s => exists s', s = (CRet (Vint pc,pcl) false false::s') /\ P m s')
...
Lemma jump_specEscape_Failure: forall raddr (P: memory -> stack -> Prop),
  HTEscape raddr [Jump]
           (fun m s => exists s0, (Vint (-1), handlerTag) ::: s0 = s /\ P m s0)
