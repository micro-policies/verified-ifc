...
Variable SysTable: syscall_table Label.
...
Variable SysTable: syscall_table observer.
...
Inductive abstract_i_equiv (o : observer) : relation (init_data (abstract_machine SysTable)) :=
  | ai_equiv : forall p d1 d2 n b
                      (STK : low_equiv_list (low_equiv_atom o) d1 d2),
                 abstract_i_equiv o (p,d1,n,b) (p,d2,n,b).

Global Program Instance AMObservation : TINI.Observation (abstract_machine SysTable) Event := {
  out e := e;                                                                              
  e_low := visible_event;
...
Notation a_step := (a_step SysTable).

Definition syscall_lowstep (SysTable:syscall_table observer) := forall id sys_info,
  SysTable id = Some sys_info ->
  forall args1 args2 res1 res2,
    map AData args1 ~~l map AData args2 ->
    si_sem sys_info args1 = Some res1 ->
    si_sem sys_info args2 = Some res2 ->
    res1 ~~a res2.

Variable hyp_syscall_lowstep : syscall_lowstep SysTable.
...
Proof.
  ...
  Case "SysCall".
    ...
    assert ((map AData args) ~~l (map AData args0)) .
...
Proof.
 ...
 Case "SysCall".
   constructor; auto.
   rewrite below_lret_adata; auto.
   intros d; rewrite in_map_iff. 
   destruct 1 as (x & X1 & X2); subst; eauto.
Qed.
...
Proof.
  ...
  exploit (a_step_instr (SysTable:=SysTable)); eauto. intros [instr1 Hinstr1].
...
Program Instance AMUnwindingSemantics 
 (SL: forall o, syscall_lowstep o SysTable) :
...
Next Obligation.
  ...
  unfold low_pc in H0; simpl in H0; try congruence.
  exploit_low. 
  inv LEa; try reflexivity.
  constructor 5; simpl; intros H; inv H;
  match goal with
    | H : ?l \_/ ?pcl <: ?o = true |- _ =>
...
Theorem abstract_noninterference :
  forall (SL: forall o, syscall_lowstep o SysTable),
  TINI.tini AMObservation.
Proof. 
  intro; apply TINI.noninterference. 
  apply AMUnwindingSemantics; auto. 
Qed.
