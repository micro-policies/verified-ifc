...
Lemma cmach_determ: 
  forall T (SysTable:syscall_table T) s e s' e' s'', 
    cstep SysTable s e s' -> 
    cstep SysTable s e' s'' -> 
    s' = s'' /\ e = e'.
Proof.
  induction 1; intros; 
  ...
  (allinv'; try split ; try reflexivity).

  Case "Dup". 
  allinv'. reflexivity.

  Case "Swap". 
  allinv'. rewrite H15 in H1. inv H1.
  reflexivity.
  ...
  Case "SysCall".
    assert (args0=args).
      eapply app_same_length_eq; eauto; congruence.
    assert (s0=s).
      eapply app_same_length_eq_rest; eauto. congruence.
    subst.
    congruence.
Qed.
