...
| step_syscall: forall id m i s pcv pcl args res sys_info, 
    index_list_Z pcv i = Some (SysCall id) -> 
    SysTable id = Some sys_info ->
    length args = sys_info.(si_arity) ->
    sys_info.(si_sem) args = Some res ->
...
Definition fetch_rule (opcode:OpCode) : (AllowModify (labelCount opcode)) :=
  match opcode with
    ...
    | OpDup => ≪ FALSE , LabPC , __ ≫ (* for now *)
    | OpSwap => ≪ FALSE , LabPC , __ ≫ (* for now *)
...
Definition tini_quasi_abstract_machine := @quasi_abstract_machine _ _ SysTable fetch_rule .
