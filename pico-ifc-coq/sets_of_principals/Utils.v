...
Lemma index_list_app_lt :
  forall T i (l1 l2 : list T),
    (i < length l1)%nat ->
    index_list i (l1 ++ l2) = index_list i l1.
Proof.
  induction i; intros; destruct l1 as [|x l1]; simpl in *; try omega.
  - reflexivity.
  - apply IHi.
    omega.
Qed.

Lemma index_list_Z_app_lt :
  forall T i (l1 l2 : list T),
    (i < Z.of_nat (length l1))%Z ->
    index_list_Z i (l1 ++ l2) = index_list_Z i l1.
Proof.
  intros.
  unfold index_list_Z.
  generalize (Z.ltb_spec0 i 0).
  intros REFL.
  destruct (i <? 0)%Z; try reflexivity.
  inv REFL.
  apply index_list_app_lt.
  rewrite Z2Nat.inj_lt in H; try omega.
  rewrite Nat2Z.id in H.
  assumption.
Qed.

Lemma index_list_Z_some_pos : forall A t m (a:A),
                                index_list_Z t m = Some a -> (0 <= t)%Z.
Proof.
  unfold index_list_Z; intros.
  destruct (t <? 0)%Z eqn:E; try congruence.
  generalize (Zlt_cases t 0); rewrite E.
  omega.
Qed.
...
Lemma index_list_Z_update_list_list'
     : forall (T : Type) (new old: list T) (i : Z),
       (i < Z.of_nat (length new))%Z -> index_list_Z i (update_list_list new old) = index_list_Z i new.
Proof.
  intros. eapply index_list_Z_update_list_list; eauto.
Qed.

Lemma index_list_app : forall A (l1:list A) i l2,
                     index_list (length l1 + i)%nat (l1 ++ l2) = index_list i l2.
Proof.
...
Lemma index_list_Z_app_general : forall A (l1:list A) i l2,
                                   (i <? 0 = false)%Z ->
                                   index_list_Z (Z.of_nat (length l1) + i) (l1 ++ l2) = index_list_Z i l2.
Proof.
  unfold index_list_Z. intros. 
  generalize (Z.ltb_spec0 i 0). intros.
  generalize (Z.ltb_spec0 (Z.of_nat (length l1) +i) 0). intros.
  destruct (Z.of_nat (length l1) + i <? 0)%Z; 
  destruct (i <? 0)%Z;  auto; (inv H0; inv H1); try (zify ; omega); try congruence.
  erewrite Z2Nat.inj_add; try omega.
  erewrite Nat2Z.id.
  eapply index_list_app.
Qed.

Lemma index_list_update_list_list_old :
  forall T (i : nat) (new old : list T),
    (i >= length new)%nat ->
    index_list i (update_list_list new old) = index_list i old.
Proof.
  intros. gdep old. gdep i.
  induction new as [|x new IH]; intros; destruct i as [|i]; destruct old as [|y old];
  simpl in *; eauto; try omega.
  - replace new with (update_list_list new nil).
    2: solve [destruct new; eauto].
    rewrite IH; try omega.
    apply index_list_nil.
  - apply IH. omega.
Qed.

Lemma index_list_Z_update_list_list_old :
  forall T (i : Z) (new old : list T),
    (i >= Z.of_nat (length new) \/ i < 0)%Z ->
    index_list_Z i (update_list_list new old) = index_list_Z i old.
Proof.
  intros.
  unfold index_list_Z.
  generalize (Z.ltb_spec0 i 0).
  ...
  destruct (i <? 0)%Z; trivial.
  inv HH.
  apply index_list_update_list_list_old.
  cut (length new <= Z.to_nat i); try omega.
  assert (Z.of_nat (length new) <= i)%Z by omega.
  rewrite Z2Nat.inj_le in H1; try omega.
...
Lemma index_list_app2 : forall (T:Type) (l1 l2: list T) (n:nat) v, 
   index_list n l1 = Some v -> index_list n (l1 ++ l2) = Some v. 
Proof.
  induction l1. 
  intros. rewrite index_list_nil in H. inv H. 
  intros. destruct n. inv H. auto. 
  simpl in H. simpl. eauto. 
Qed.

Lemma index_list_Z_app2 : forall (T:Type) (l1 l2: list T) (i:Z) v,
   index_list_Z i l1 = Some v -> index_list_Z i (l1 ++ l2) = Some v. 
Proof.
  intros. unfold index_list_Z in *. destruct (i <? 0). inv H. 
  eapply index_list_app2; eauto. 
Qed.
...
Lemma update_list_gte_app :
  forall T n (x : T) l1 l2 l l',
    length l1 <= n ->
    update_list n x (l1 ++ l2) = Some l ->
    update_list (n - length l1) x l2 = Some l' ->
    l = l1 ++ l'.
Proof.
  induction n as [|n IH]; intros x l1 l2 l l' H1 H2 H3;
  destruct l1 as [|x' l1]; auto; simpl in *; try omega;
  try congruence.
  destruct (update_list n x (l1 ++ l2)) as [l''|] eqn:E; try congruence.
  inv H2.
  f_equal. 
  eapply IH; eauto.
  omega.
...
Lemma update_list_Z_gte_app :
  forall T n (x : T) l1 l2 l l',
    (Z.of_nat (length l1) <= n)%Z ->
    update_list_Z n x (l1 ++ l2) = Some l ->
    update_list_Z (n - Z.of_nat (length l1)) x l2 = Some l' ->
    l = l1 ++ l'.
Proof.
  unfold update_list_Z.
  intros T n x l1 l2 l l' H1 H2 H3.
  destruct (n <? 0)%Z; try congruence.
  destruct (n - Z.of_nat (length l1) <? 0)%Z; try congruence.
  eapply update_list_gte_app with (n := Z.to_nat n); eauto.
