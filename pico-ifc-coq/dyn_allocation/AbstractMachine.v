...
Inductive a_step : a_state -> option (Event T) -> a_state -> Prop :=
| step_nop : forall m i s pcv pcl
    (INSTR: index_list_Z pcv i = Some Noop),
    a_step (AState m i s (pcv,pcl)) None (AState m i s (pcv+1,pcl))
...
| step_ret: forall m i s pcv pcl pcv' pcl' s' 
    ...
    a_step (AState m i s (pcv,pcl)) None (AState m i s' (pcv',pcl'))
