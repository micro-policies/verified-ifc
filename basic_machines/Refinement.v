...
Let low_compatible o1 o2 :=
  forall e1 e2,
    ...
    (TINI.e_low o1 e1 <-> TINI.e_low o2 e2).
...
Proof.
  ...
  match goal with
    | |- TINI.ti_trace_indist (?E1 :: _) (?E2 :: _) =>
      assert (Heq : TINI.a_equiv _ o2 (E E1) (E E2))
        by (eapply H3; eauto; try reflexivity)
  end.
...
Hypothesis lockstep : forall s11 s21 a2 s22,
                        match_states s11 s21 ->
                        step S2 s21 a2 s22 ->
                        exists a1 s12,
                          step S1 s11 a1 s12 /\ match_actions a1 a2 /\ match_states s12 s22.
